...
/** return true when the robot is forcing.
*/
bool CMDMOTOR::isForcing()
{
  return pid->isClamp();
}
 ...
 */
void CMDMOTOR::setSpeed(signed int mypoint) {
	setPoint(mypoint);
}
/** set the length, the distance when the speed is keeped.
 */
void CMDMOTOR::setLength(signed int mypoint) {

	if (LengthEnabled)
		length += mypoint;
	else
		length = getEncoder()->getValue() + mypoint;
	LengthEnabled = true;
LengthReached=false;preverror=mypoint;

}
void CMDMOTOR::stopLength()
{
    LengthEnabled=false;
}


/** return true once we reach the length set
*/
boolean CMDMOTOR::isLengthReached()
{
  int error=  length - getEncoder()->getValue();
...
/** deprecated : set the speed
 */
void CMDMOTOR::setPoint(signed int mypoint) {
...
CMDMOTOR::CMDMOTOR(int INCA, int INCB, int MP, int MM) {
	...
	pidpos = new PID(&position, &point, &length, 1000, 0, 0, DIRECT);
...
void CMDMOTOR::setRefreshRateUs(uint32_t intervalTime //!< duration between 2 topic in Micro Seconde
	...
	if (pidpos != 0)
		pidpos->SetSampleTime(intervalTime / 1000);
}
...
void CMDMOTOR::calib() {
	...
	stop();
	signed int pwm0 = -1;
	while (i > pwm0) { // search again the K0, on bad motoreductor, these is several blocking point
		pwm0 = i;
...
void CMDMOTOR::setPin(int INCA, int INCB, int MP, int MM) {
	setPin(INCA, INCB, MP, MM, -1,false);
}
void CMDMOTOR::setPin(int INCA, int INCB, int MP, int MM, int EN,boolean invert) {
	pinINCA = INCA;
	pinINCB = INCB;
	pinMP = MP;
	pinMM = MM;
...
void CMDMOTOR::SetPwmLimits(signed int Min, signed int Max) {
	pid->SetOutputLimits(Min, Max);
}
void CMDMOTOR::SetSpeedLimits(double Min, double Max) {
	pidpos->SetOutputLimits(Min, Max);
}
...
void CMDMOTOR::loop() {
	...
	int res = false;
	if (LengthEnabled) {
		position = getEncoder()->getValue();

		res = pidpos->Compute();

		/*   if (abs(e)<getPoint()/100)
		 {
		 setSpeed(e*100);
		 } 
		 */
		/*     if (abs(length-getEncoder()->getValue())<=abs(10))
		 {
		 LengthEnabled=false;setSpeed(0);
	...
	if (disten) {
		if (((point > 0) && (getEncoder()->getValue() >= currentDistTarget))
				|| ((getEncoder()->getValue() <= currentDistTarget)
						&& (point < 0))) {  // next point
			if (fifoin == fifoout) {
				stop();
				disten = false;
				...
				fifoout++;
				fifoout = fifoout >= SizeFIFO ? 0 : fifoout;
			}
		}
	}
